...
class GoldBuySellLinkSection extends StatefulWidget {
  final GoldBuySellLinkState state;
  final int? linkedSellId;
  final VoidCallback onClear;
  final Function(int id) onSearchById;
  final Function(String term) onSearchByClient;
  final Function(SellFindModel sell) onSelectSell;

  const GoldBuySellLinkSection({
    Key? key,
    required this.state,
  ...
  @override
  State<GoldBuySellLinkSection> createState() => _GoldBuySellLinkSectionState();
}

class _GoldBuySellLinkSectionState extends State<GoldBuySellLinkSection> {
              ...
              if (widget.linkedSellId != null)
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: AppColors.goldColor.withOpacity(0.25),
                    ),
                  ),
                  child: Row(
                    ...
                    children: [
                      Text(
                        '${'gold_buy.linkToPriorSell'.tr()} #${widget.linkedSellId}',
                        style: AppFonts.body(
                          fontSize: 11.sp,
            ...
            children: [
              Expanded(
                child: CustomTextFormField(
                  ...
                  labelText: 'gold_buy.phoneOrEmail'.tr(),
                  height: 66.h,
                  borderRadius: BorderRadius.circular(12.r),
                  borderColor: isDark
                      ? Colors.white.withOpacity(0.08)
  ...
  Widget _buildStateContent(bool isDark) {
    return widget.state.maybeWhen(
      initial: () => Padding(
        ...
        child: Text(
          'gold_buy.searchByOrderIdOrClient'.tr(),
          style: AppFonts.body(
            fontSize: 12.sp,
            color: isDark
                ? AppColors.textGreyColor
      ...
      foundClientSells: (clientSells) => Padding(
        padding: EdgeInsets.only(top: 10.h),
        child: clientSells.goldSells.isEmpty
            ? Text(
                'gold_buy.noSellOrdersForClient'.tr(),
                style: AppFonts.body(
                  fontSize: 12.sp,
                  color: isDark
                      ? AppColors.textGreyColor
            ...
            : Column(
                children: List.generate(
                  clientSells.goldSells.length,
                  (index) => Padding(
                    padding: EdgeInsets.only(
                      bottom: index == clientSells.goldSells.length - 1
                          ? 0
                          : 8.h,
                    ),
                    child: _SellCard(
                      sell: clientSells.goldSells[index],
                      onTap: () =>
                          widget.onSelectSell(clientSells.goldSells[index]),
                    ),
                  ),
                ),
              ),
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text(
                    'gold_buy.sellRecord'.tr() + ' #${sell.id}',
                    style: AppFonts.heading(
                      fontSize: 14.sp,
                      fontWeight: FontWeight.w700,
                      color: isDark
