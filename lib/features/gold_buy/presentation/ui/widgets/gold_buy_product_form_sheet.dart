...
class GoldBuyProductFormSheet extends StatefulWidget {
  ...
  @override
  State<GoldBuyProductFormSheet> createState() =>
      _GoldBuyProductFormSheetState();
}

class _GoldBuyProductFormSheetState extends State<GoldBuyProductFormSheet> {
  GoldCaratModel? selectedCarat;
  GoldBoxModel? selectedBox;
  ...
  void _onCaratSelected(GoldCaratModel? carat) {
    setState(() {
      selectedCarat = carat;
      if (carat != null) {
        gramPriceController.text = (carat.price - 50).toStringAsFixed(2);
        _updateBuyPrice();
      }
    });
  }
  ...
  void _submit() {
    ...
    if (grams == null || grams <= 0) {
      ToastService.show('gold_buy.gramsMustBeGreater'.tr());
      return;
    }

    final loss = double.tryParse(lossController.text) ?? 0;
    if (loss < 0) {
      ToastService.show('gold_buy.lossCannotBeNegative'.tr());
      return;
    }

    if (loss >= grams) {
      ToastService.show('gold_buy.lossMustBeLessThanGrams'.tr());
      return;
    }

    final gramPrice = double.tryParse(gramPriceController.text);
    if (gramPrice == null || gramPrice <= 0) {
      ToastService.show('gold_buy.gramPriceMustBeGreater'.tr());
      return;
    }
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<GoldCaratModel>(
              ...
              items: widget.carats
                  .map(
                    (carat) => DropdownMenuItem<GoldCaratModel>(
                      value: carat,
                      child: Text(
            ...
            DropdownButtonFormField<GoldBoxModel>(
              ...
              items: widget.boxes
                  .map(
                    (box) => DropdownMenuItem<GoldBoxModel>(
                      value: box,
                      child: Text(box.name),
                    ),
                  )
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text('gold_buy.addProduct'.tr()),
              ),
            ),
          ],
        ),
