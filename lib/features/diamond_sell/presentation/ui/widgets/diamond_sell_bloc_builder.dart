...
import 'package:kenooz_worker_app/features/diamond_sell/presentation/ui/widgets/diamond_sell_shimmer.dart';

class DiamondSellBlocBuilder extends StatelessWidget {
  final Widget Function(DiamondSellPreloadData data) successBuilder;
  final VoidCallback onRetry;

  const DiamondSellBlocBuilder({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DiamondSellPreloadCubit, DiamondSellPreloadState>(
      builder: (context, state) {
        return state.maybeWhen(
          loading: () => const DiamondSellShimmer(),
          success: successBuilder,
          error: (messages) => _ErrorView(
            message: messages.join('\n'),
            onRetry: onRetry,
          ),
          orElse: () => const DiamondSellShimmer(),
        );
      },
    );
  }
