...
import 'package:kenooz_worker_app/features/silver_double_sell/presentation/ui/widgets/silver_double_sell_box_tab.dart';
import 'package:kenooz_worker_app/features/silver_double_sell/presentation/ui/widgets/silver_double_sell_buy_section.dart';
import 'package:kenooz_worker_app/features/silver_double_sell/presentation/ui/widgets/silver_double_sell_client_section.dart';
...
import 'package:kenooz_worker_app/features/silver_double_sell/presentation/ui/widgets/silver_double_sell_inside_tab.dart';
import 'package:kenooz_worker_app/features/silver_double_sell/presentation/ui/widgets/silver_double_sell_outside_tab.dart';

const Color _silverAccent = Color(0xFF9E9E9E);
const Color _buyAccent = Color(0xFF4CAF50);

/// Desktop Silver Double Sell — mirrors GoldDoubleSellDesktopScreen but uses
/// silver cubits, models, sub-widgets, and vendor sheet. All pricing math,
/// validation, and the submit payload structure are preserved exactly from
/// the mobile screen.
class SilverDoubleSellDesktopScreen extends StatefulWidget {
  const SilverDoubleSellDesktopScreen({super.key});

  @override
  State<SilverDoubleSellDesktopScreen> createState() =>
      _SilverDoubleSellDesktopScreenState();
}

class _SilverDoubleSellDesktopScreenState
    extends State<SilverDoubleSellDesktopScreen> {
  @override
  void initState() {
    super.initState();
    context.read<SilverDoubleSellPreloadCubit>().fetchPreloadData();
  }

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    return BlocListener<SilverDoubleSellSubmitCubit, SilverDoubleSellSubmitState>(
      listener: (context, state) {
        state.whenOrNull(
          ...
          success: (data) {
            EasyLoading.dismiss();
            successSnackBar(
              msg: 'silver_double_sell.submitSuccess'.tr(),
              context: context,
            );
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            Expanded(
              child: BlocBuilder<SilverDoubleSellPreloadCubit,
                  SilverDoubleSellPreloadState>(
                builder: (context, state) => state.maybeWhen(
                  ...
                  error: (messages) => _DesktopError(
                    ...
                    onRetry: () => context
                        .read<SilverDoubleSellPreloadCubit>()
                        .fetchPreloadData(),
                  ),
                  success: (data) =>
                      _SilverDoubleSellDesktopFormView(preloadData: data),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'silver_double_sell.subtitle'.tr(),
                    style: AppFonts.body(
                      fontSize: 12,
                      color: theme.colorScheme.onSurface
...
class _SilverDoubleSellDesktopFormView extends StatefulWidget {
  const _SilverDoubleSellDesktopFormView({required this.preloadData});
  final SilverDoubleSellPreloadData preloadData;

  @override
  State<_SilverDoubleSellDesktopFormView> createState() =>
      _SilverDoubleSellDesktopFormViewState();
}

class _SilverDoubleSellDesktopFormViewState
    extends State<_SilverDoubleSellDesktopFormView> {
  ...
  late List<SilverVendorModel> sellVendors;
  ...
  double _recalculateProfitFromEditedPrice(
    ...
    if (item.type == 'inside' || item.type == 'outside') {
  ...
  Future<void> _openCreateClientSheet() async {
    ...
    context.read<SilverClientSearchCubit>().createClient(request);
  }
  ...
  void _submit(double finalTotal) {
    ...
    final request = SilverDoubleSellRequestModel(
      ...
      buySilverProducts: _buyItems.map((item) => item.payload).toList(),
      deductionAccounts:
          _buyPaymentEntries.map((e) => e.toRequestMap()).toList(),
    );
    context.read<SilverDoubleSellSubmitCubit>().submitSell(request);
  }
  ...
  String? _validateBeforeSubmit(double finalTotal) {
    ...
    if (_sellPaymentEntries.isEmpty) {
      return 'gold_double_sell.addAtLeastOneSellPaymentAccount'.tr();
    }
    if (_buyItems.isNotEmpty && _buyPaymentEntries.isEmpty) {
      return 'gold_double_sell.addDeductionAccountForBuyItems'.tr();
    }
    if (finalTotal <= 0) {
  ...
  @override
  Widget build(BuildContext context) {
    final totals = SilverDoubleSellPriceCalculator.calculateTotals(
    ...
    return MultiBlocListener(
      listeners: [
        BlocListener<SilverClientSearchCubit, SilverClientSearchState>(
          listener: (context, state) {
            state.whenOrNull(
  ...
  Widget _buildFormColumn(
      ({double sellTotal, double buyTotal, double net, double taxAmount, double finalTotal}) totals) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        DesktopCard(
          padding: const EdgeInsets.all(16),
          child: BlocBuilder<SilverClientSearchCubit, SilverClientSearchState>(
            builder: (context, state) {
              return SilverDoubleSellClientSection(
                searchController: _clientSearchController,
                state: state,
                selectedClient: _selectedClient,
                onSearch: () => context
                    .read<SilverClientSearchCubit>()
                    .searchClient(_clientSearchController.text),
                onOpenCreateClientSheet: _openCreateClientSheet,
                onClear: () {
                  setState(() => _selectedClient = null);
                  context.read<SilverClientSearchCubit>().clearClient();
                },
              );
            },
          ),
        ...
        DesktopCard(
          ...
          child: Column(
            ...
            children: [
              ...
              BlocBuilder<SilverProductLookupCubit, SilverProductLookupState>(
                builder: (context, productState) {
                  return DoubleSellItemTabs(
                    insideTab: SilverDoubleSellInsideTab(
                      ...
                      onLookup: () {
                        final id = int.tryParse(
                            ...
                            0;
                        context
                            .read<SilverProductLookupCubit>()
                            .lookupProduct(id);
                      },
                      onClearLookup: () =>
                          context.read<SilverProductLookupCubit>().clearLookup(),
                      onAddItem: (item) =>
                          setState(() => _sellItems.add(item)),
                    ),
                    boxTab: SilverDoubleSellBoxTab(
                      carats: widget.preloadData.carats,
                      kinds: widget.preloadData.kinds,
                    ...
                    outsideTab: SilverDoubleSellOutsideTab(
                      carats: widget.preloadData.carats,
                      kinds: widget.preloadData.kinds,
                      vendors: List.of(sellVendors),
                      onCreateVendor: _openCreateVendorSheet,
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton(
            ...
            onPressed: onSubmit,
            child: Text(
              'silver_double_sell.submitSilverDoubleSell'.tr(),
