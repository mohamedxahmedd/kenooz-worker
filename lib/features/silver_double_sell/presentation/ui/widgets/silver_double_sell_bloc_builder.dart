...
import 'package:kenooz_worker_app/features/silver_double_sell/presentation/ui/widgets/silver_double_sell_shimmer.dart';

class SilverDoubleSellBlocBuilder extends StatelessWidget {
  final Widget Function(SilverDoubleSellPreloadData data) successBuilder;
  final VoidCallback onRetry;

  const SilverDoubleSellBlocBuilder({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<SilverDoubleSellPreloadCubit, SilverDoubleSellPreloadState>(
      builder: (context, state) {
        return state.maybeWhen(
          loading: () => const SilverDoubleSellShimmer(),
          success: successBuilder,
          error: (messages) => _ErrorView(
            message: messages.join('\n'),
            onRetry: onRetry,
          ),
          orElse: () => const SilverDoubleSellShimmer(),
        );
      },
    );
  }
      ...
      child: Padding(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.error_outline_rounded, color: AppColors.errorColor, size: 34.sp),
