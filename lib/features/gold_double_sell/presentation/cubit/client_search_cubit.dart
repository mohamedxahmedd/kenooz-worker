...
class ClientSearchCubit extends Cubit<ClientSearchState> {
  ...
  ClientSearchCubit(this._repo) : super(const ClientSearchState.initial());

  Future<void> searchClient(String term) async {
    final normalizedTerm = term.trim();
    if (normalizedTerm.isEmpty) {
      emit(const ClientSearchState.initial());
      return;
    }

    emit(const ClientSearchState.loading());

    final response = await _repo.searchClient(normalizedTerm);
    response.when(
      success: (client) => emit(ClientSearchState.found(client)),
      failure: (failure) {
        if (_isNotFound(failure.errorModel)) {
          emit(const ClientSearchState.notFound());
          return;
        }
  ...
  Future<void> createClient(CreateClientRequestModel request) async {
    ...
    response.when(
      success: (client) => emit(ClientSearchState.created(client)),
      failure: (failure) {
        emit(
          ClientSearchState.error(
            messages: _extractMessages(failure.errorModel),
